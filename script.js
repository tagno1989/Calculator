...
function handleKeyboardInput(e) {
    ...
    if (e.key === '=' || e.key === 'Enter') calculator.compute(e.key)
    if(e.key === 'Backspace') calculator.delete(e.key)
    if(e.key === 'Escape') calculator.clear(e.key)
    if(e.key === '+' || e.key === '-' || e.key === '*' || e.key === '/') 
    calculator.chooseOperation(convertOperator(e.key))
    calculator.updateDisplay(e.key)
}

function convertOperator(operation) {
    if (operation === '/') return '÷'
